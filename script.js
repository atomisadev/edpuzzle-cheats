...
if (typeof document.dev_env != "undefined") {
  ...
  base_url =
    "https://raw.githubusercontent.com/ading2210/edpuzzle-answers/main";
}
...
function init() {
  if (window.location.hostname == "edpuzzle.hs.vc") {
    ...
    /https{0,1}:\/\/edpuzzle.com\/assignments\/[a-f0-9]{1,30}\/watch/.test(
...
function handleCanvasURL() {
  ...
  http_get(url, function () {
    ...
    http_get(url2, function () {
      ...
      alert(
        `Please re-run this script in the newly opened tab. If nothing happens, then allow popups on Canvas and try again.`
      );
      open(url3);
    });
  });
...
function handleSchoologyURL() {
  ...
  http_get(url, function () {
    alert(
      `Please re-run this script in the newly opened tab. If nothing happens, then allow popups on Schoology and try again.`
    );
...
function getAssignment(callback) {
  ...
  if (typeof assignment_id == "undefined") {
    alert(
      "Error: Could not infer the assignment ID. Are you on the correct URL?"
    );
    return;
  }
  ...
  http_get(url1, function () {
    ...
    if (("" + this.status)[0] == "2") {
      ...
      alert(
        `Error: Status code ${this.status} recieved when attempting to fetch the assignment data.`
      );
    }
  });
}
...
function openPopup(assignment) {
  ...
  if (teacher_assignment.preferences.dueDate == "") {
    ...
    deadline_text =
      "due on " +
      new Date(teacher_assignment.preferences.dueDate).toDateString();
  }
  ...
  <hr>
  <p style="font-size: 12px">Made by: <a target="_blank" href="https://github.com/atomisadev">atomisadev</a> on Github | Website: <a target="_blank" href="https://edpuzzle.hs.vc">edpuzzle.hs.vc</a> | Source code: <a target="_blank" href="https://github.com/ading2210/edpuzzle-answers">ading2210/edpuzzle-answers</a></p>
      ...
      for (let j = 0; j < question.choices.length; j++) {
        ...
        if (typeof choice.body != "undefined") {
          ...
          if (choice.isCorrect == true) {
            choices_lines.push(
              `<li class="choice choice-correct">${item_html}</li>`
            );
          } else {
